...
type: algorithm
status: stable
tags: [dsa/algorithm, dsa/counting-sort]
canonical: true
...
## Overview
Non-comparison sort that counts frequencies over a bounded integer key range.
...
## Complexity
O(n+k) time and O(k) space for key range k.
...
## Applications
Small integer domains, radix sort subroutine, stable bucket placement.
