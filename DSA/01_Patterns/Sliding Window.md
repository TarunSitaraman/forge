...
type: pattern
status: stable
tags: [dsa/pattern, dsa/sliding-window]
canonical: true
related: [[[Python - Sliding Window]], [[Two Pointers]], [[Prefix Sum]], [[Sliding Window Cheat Sheet]]]
---
# Sliding Window

## Intuition
Maintain a moving contiguous range while updating state incrementally instead of recomputing each interval.

## When To Use
Contiguous subarrays or substrings, at most or at least constraints, frequency maps, running sums.
...
## Implementation
Use the canonical implementation link when one exists: [[Python - Sliding Window]]. Keep problem-specific code inside [[Problem Index]] pages and avoid copying full template code here.

## Complexity
Usually O(n) time because each boundary moves monotonically; O(k) state for tracked counts.
