...
type: pattern
status: stable
tags: [dsa/pattern, dsa/difference-array]
canonical: true
related: [[[Python - Difference Array]], [[Prefix Sum]], [[Sweep Line]], [[Complexities Cheat Sheet]]]
---
# Difference Array

## Intuition
Represent range updates by marking boundary deltas and reconstructing values with a prefix pass.

## When To Use
Many range increments, interval additions, offline updates, capacity accumulation.
...
## Implementation
Use the canonical implementation link when one exists: [[Python - Difference Array]]. Keep problem-specific code inside [[Problem Index]] pages and avoid copying full template code here.

## Complexity
O(1) per range update and O(n) reconstruction; space O(n).
