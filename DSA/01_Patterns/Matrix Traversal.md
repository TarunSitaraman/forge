...
type: pattern
status: stable
tags: [dsa/pattern, dsa/matrix-traversal]
canonical: true
related: [[[Python - Matrix Traversal]], [[BFS]], [[DFS]], [[Graph Traversal]], [[Graphs Cheat Sheet]]]
---
# Matrix Traversal

## Intuition
Traverse two-dimensional grids while preserving boundary checks, direction rules, and visited state.

## When To Use
Islands, flood fill, shortest grid path, spiral traversal, multi-source BFS.
...
## Implementation
Use the canonical implementation link when one exists: [[Python - Matrix Traversal]]. Keep problem-specific code inside [[Problem Index]] pages and avoid copying full template code here.

## Complexity
O(rows*cols); queue/stack can hold O(rows*cols) cells.
...
## Common Bugs
[[Boundary Errors]], [[Missing Visited Set]]
