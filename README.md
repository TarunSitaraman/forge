...
| | What it is |
|---|---|
| **The engine** (`engine/`) | A Python knowledge OS. ~18,000 lines, 810 offline tests. Reads the vault, derives a provenance-aware model from it, and never writes back without explicit approval. |
...
python -m pytest tests -q   # 810 tests, offline
```
