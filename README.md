...
| | What it is |
|---|---|
| **The engine** (`engine/`) | A Python knowledge OS. ~18,000 lines, 842 offline tests. Reads the vault, derives a provenance-aware model from it, and never writes back without explicit approval. |
...
```bash
python -m pytest tests -q        # 842 tests, fully offline, no model needed
...
├── tests/  scripts/        842 tests; demos and per-phase validation scripts.
...
python -m pytest tests -q   # 842 tests, offline
```
