...
| | What it is |
|---|---|
| **The engine** (`engine/`) | A Python knowledge OS. ~18,000 lines, 830 offline tests. Reads the vault, derives a provenance-aware model from it, and never writes back without explicit approval. |
...
```bash
python -m pytest tests -q        # 830 tests, fully offline, no model needed
...
├── tests/  scripts/        830 tests; demos and per-phase validation scripts.
...
python -m pytest tests -q   # 830 tests, offline
```
