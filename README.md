...
| | What it is |
|---|---|
| **The engine** (`engine/`) | A Python knowledge OS. ~18,000 lines, 829 offline tests. Reads the vault, derives a provenance-aware model from it, and never writes back without explicit approval. |
...
characterisation** — five cases cannot establish a rate, and the hosted path
...
reasoning: a 401 probe was read as evidence that the hosted request shape was
...
```bash
python -m pytest tests -q        # 829 tests, fully offline, no model needed
...
├── tests/  scripts/        829 tests; demos and per-phase validation scripts.
...
python -m pytest tests -q   # 829 tests, offline
```
