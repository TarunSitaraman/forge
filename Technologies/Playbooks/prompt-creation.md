...
`../Prompt-Library/` entry — one that encodes actual expertise, not
...
- You've actually used this prompt (or a rough version of it)
  ...
  library yet, per `../Prompt-Library/_index.md`'s admission rule.
...
## Outputs
- A new Markdown file in the correct `../Prompt-Library/<category>/`
...
- [ ] Correct category identified (or a genuine new category, only if
      2+ prompts justify it — see `../_index.md`'s rule on new
      subfolders)
...
8. Update the category's entry in `../Prompt-Library/_index.md` if
   this is a new category, or the category folder's own scannability if
   it's a large addition.

## AI prompts
- `../Prompt-Library/Prompt-Engineering/prompt-failure-diagnosis.md` — to sharpen the prompt if early drafts produce inconsistent output
- `../Prompt-Library/Prompt-Engineering/system-prompt-design-review.md` — if the new prompt is meant to configure an agent's persistent behavior rather than a single task
