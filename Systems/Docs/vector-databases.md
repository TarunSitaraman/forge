...
- **Treating vector search as a complete solution** for retrieval quality
  ...
  `Systems/Docs/rag.md`) — the vector database can only return what was
  indexed well.
...
- `Systems/Docs/rag.md` for how vector search fits into the broader
  retrieval pipeline.
- `Systems/Prompt-Library/Vector-Databases/vector-db-selection.md` and
